...
/* ---------------------------------------------------
   GLOBAL CORS 설정 (무조건 모든 요청에 적용됨)
--------------------------------------------------- */
app.use(
  cors({
    origin: ['https://oxbit.app', 'https://otp.oxbit.app'],
...
// OPTIONS(Preflight) 요청 처리 — 여기서 CORS 반드시 허용
app.options('*', (req, res) => {
  res.header('Access-Control-Allow-Origin', '*');
  res.header('Access-Control-Allow-Methods', 'POST, GET, OPTIONS');
  res.header(
    'Access-Control-Allow-Headers',
    'Content-Type, Authorization, x-client-info, apikey, x-proxy-token',
  );
