...
function TimelineItem({ item, prevKey, nextKey }: { item: BreakingItem; prevKey?: string; nextKey?: string }) {
  ...
  // Load synced like/comment counts for admin posts
  React.useEffect(() => {
    let mounted = true
    const load = async () => {
      if (!item.id) return
  ...
  const load = React.useCallback(async () => {
    try {
      ...
      const toItem = (n: any): BreakingItem => {
        ...
        // Generate safe URL segment to avoid %2F (which can be decoded to '/').
        ...
        return { key, time: `${hh}:${mm}`, title: n.title, body: n.summary || n.title, tag: n.source, url: n.url, id: n.id }
      }
