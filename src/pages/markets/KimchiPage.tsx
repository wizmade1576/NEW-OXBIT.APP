          ...
          <CardContent className="p-3 sm:p-6 pt-0">
            ...
            <div className="hidden sm:block">
              ...
              <div className="mt-2 text-sm text-muted-foreground">
                BTC 기준 스프레드 {fmtKRW(btc?.spread)} (국내 {fmtKRW(btc?.domestic)} · 해외 {fmtKRW(btc?.foreignKrw)})
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card className="bg-[#121212] border-neutral-800">
        ...
        <CardContent>
          ...
          <div className="sm:hidden space-y-3 mt-3">
            ...
            {displayed.map(r => (
              ...
              >
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className={`text-[15px] font-bold ${pctClass(r.premium)}`}>
                    {fmtPct(r.premium)}
                  </div>
                </div>
            ...
            {loading ? (
              <div className="text-xs text-neutral-400 text-center mt-2">업데이트 중...</div>
            ) : null}
...
async function listDomesticSymbols(exchange: Domestic): Promise<string[]> {
  try {
    ...
    if (exchange === 'Upbit') {
      ...
      const j = await r.json();
      return j
        .filter((it: any) => String(it.market || '').startsWith('KRW-'))
        .map((it: any) => String(it.market).replace('KRW-', ''));
    }
