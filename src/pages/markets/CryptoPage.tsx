...
export default function CryptoPage() {
  ...
  const [fav, setFav] = React.useState<Record<string, boolean>>(() => {
    try {
      return JSON.parse(localStorage.getItem('fav_coins') || '{}')
    } catch {
      return {}
    }
  })
  ...
  const displayed = React.useMemo(() => {
    ...
    return displayedRaw.slice(start, start + pageSize)
  }, [displayedRaw, page])
  ...
  const handleFavClick = (e: React.MouseEvent<HTMLButtonElement>, id: string) => {
    ...
    setFav((prev) => {
      ...
      try {
        localStorage.setItem('fav_coins', JSON.stringify(nx))
      } catch {}
      return nx
    })
    ...
    <section className="space-y-4">
      ...
      <div className="flex items-center gap-1 px-3 w-full sm:hidden">
        ...
        <div className="flex items-center gap-1 w-[55%] justify-end flex-nowrap flex-none">
          <button
            ...
            className={`px-2 py-1 rounded border border-neutral-700 text-xs whitespace-nowrap shrink-0 ${
              currency === 'USD' ? 'bg-emerald-600/20 text-emerald-300' : 'bg-[#1a1a1a] hover:bg-[#1e1e1e]'
            }`}
          >
            USD
          </button>
          <button
            ...
            className={`px-2 py-1 rounded border border-neutral-700 text-xs whitespace-nowrap shrink-0 ${
              currency === 'KRW' ? 'bg-emerald-600/20 text-emerald-300' : 'bg-[#1a1a1a] hover:bg-[#1e1e1e]'
            }`}
          >
            KRW
          </button>
          <button
            ...
            className={`px-2 py-1 rounded border border-neutral-700 text-xs whitespace-nowrap shrink-0 ${
              category === 'FAV' ? 'bg-emerald-600/20 text-emerald-300' : 'bg-[#1a1a1a] hover:bg-[#1e1e1e]'
            }`}
          >
            즐겨찾기
          </button>
      ...
      <div className="hidden sm:flex flex-wrap items-center gap-2">
        <input
          ...
          placeholder="코인 검색 (예: btc, eth)"
          className="px-3 py-2 rounded border border-neutral-700 bg-[#1a1a1a] focus:outline-none w-full sm:w-auto"
        />
      ...
      {false && status === 'rate_limited' && (
        <div className="text-xs text-amber-300">API rate limit: {remain}s 후 재시도 예정</div>
      )}
        ...
        <CardContent>
            ...
            <table className="min-w-full text-[13px] sm:text-sm">
                  ...
                  {[
                    ...
                    <th key={c.key} className="px-3 py-2 text-left select-none">
                      <button
                        onClick={() =>
                          setSort((s) => ({
                            key: c.key === 'spark' ? 'market_cap' : c.key,
                            dir: s.key === c.key && s.dir === 'desc' ? 'asc' : 'desc',
                          }))
                        }
              ...
              <tbody>
                {displayed.map((c) => (
                  <tr
                    key={c.id}
                    className="border-t border-neutral-800 hover:bg-[#1e1e1e] cursor-pointer"
                    onClick={(e) => openDetail(e, c)}
                  >
                    <td className="px-3 py-2">{(c as any).market_cap_rank || '-'}</td>
                    <td className="px-3 py-2">
                      <button
                        onClick={(e) => handleFavClick(e, c.id)}
                        className={`inline-flex items-center justify-center p-0.5 rounded-full text-sm hover:text-yellow-400 transform transition duration-200 ${
                          favPulse[c.id] ? 'scale-125 ring-2 ring-yellow-400' : ''
                        } ${fav[c.id] ? 'text-yellow-400' : 'text-gray-500'}`}
                    ...
                    <td className="px-3 py-2">
                      {formatCurrency(convert(c.current_price, currency, usdkrw), currency)}
                    </td>
                    <td className={`px-3 py-2 ${pctClass(c.price_change_percentage_24h)}`}>
                      {fmtPct(c.price_change_percentage_24h)}
                    </td>
                    <td className={`px-3 py-2 ${pctClass(c.price_change_percentage_7d_in_currency)}`}>
                      {fmtPct(c.price_change_percentage_7d_in_currency)}
                    </td>
                    <td className="px-3 py-2">
                      {formatCurrency(convert(c.total_volume, currency, usdkrw), currency)}
                    </td>
                    <td className="px-3 py-2">
                      {formatCurrency(convert(c.market_cap, currency, usdkrw), currency)}
                    </td>
                    <td className="px-3 py-2">
                      <SparkLine
          ...
          <div className="mt-3 flex items-center justify-end gap-2 text-sm">
            <button
              ...
              className="px-2 py-1 rounded border border-neutral-700 bg-[#1a1a1a] hover:bg-[#1e1e1e]"
            >
              Prev
            </button>
            <span className="text-gray-400">
              Page {page} / {Math.max(1, Math.ceil(displayedRaw.length / pageSize))}
            </span>
            <button
              onClick={() => setPage((p) => Math.min(Math.ceil(displayedRaw.length / pageSize), p + 1))}
              className="px-2 py-1 rounded border border-neutral-700 bg-[#1a1a1a] hover:bg-[#1e1e1e]"
            >
              Next
            </button>
            ...
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <div className="text-white font-semibold">
                  {detail.name} <span className="uppercase text-gray-400 text-xs">{detail.symbol}</span>
                </div>
              </div>
              <button
                onClick={() => { setDetail(null); setAnchor(null) }}
                className="px-2 py-1 rounded bg-[#1a1a1a] hover:bg-[#1e1e1e]"
              >
                닫기
              </button>
            </div>
            <div className="space-y-2 text-sm">
              <div className="flex items-center justify-between">
                ...
                <span className="text-white">
                  {formatCurrency(convert(detail.current_price, currency, usdkrw), currency)}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-400">24h 최고</span>
                <span className="text-emerald-400">
                  {formatCurrency(convert(detail.high_24h, currency, usdkrw), currency)}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-400">24h 최저</span>
                <span className="text-red-400">
                  {formatCurrency(convert(detail.low_24h, currency, usdkrw), currency)}
                </span>
              </div>
            </div>
            <div className="mt-3">
              <SparkLine
                data={detail.sparkline_in_7d?.price || []}
                up={(detail.price_change_percentage_7d_in_currency || 0) >= 0}
              />
            </div>
          </div>
