        ...
        <CardHeader>
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="inline-flex rounded-md border border-neutral-700 overflow-hidden">
              {tabs.map(t => (
                <button key={t.key} onClick={()=>setTab(t.key)} className={`px-3 py-1.5 text-sm ${tab===t.key ? 'bg-emerald-600/20 text-emerald-300' : 'bg-[#1a1a1a] hover:bg-[#1e1e1e]'}`}>{t.label}</button>
              ))}
            </div>
          </div>
        </CardHeader>
