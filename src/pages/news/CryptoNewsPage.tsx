...
import { fetchCrypto, getProvider, type NewsItem } from '../../lib/news/providers'
  ...
  return (
    <section className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          <h3 className="text-xl font-semibold">암호화폐 뉴스</h3>
          <p className="text-muted-foreground text-sm">주요 이슈/큰손 트렌드</p>
        </div>
          ...
          <div className="space-y-4">
            {(function(){
              ...
              if (activeFilters.length) {
                list = list.filter(n => activeFilters.some(f => n.title.toLowerCase().includes(f.toLowerCase())))
              }
