...
import { fetchStocks, getProvider, type NewsItem, normalizeNewsUrl } from '../../lib/news/providers'
...
export default function GlobalStocksNewsPage() {
  ...
  const LS_KEY = 'news:global:v2'
  const CACHE_TTL_MS = 1000 * 60 * 10

  // Hydrate from localStorage and background refresh
  React.useEffect(() => {
    try {
          ...
          if (age > CACHE_TTL_MS) {
            ;(async () => {
              ...
              try {
                ...
                const { items: more, cursor: nextCursor, nextPage } = await fetchStocks(provider === 'reddit' ? undefined : provider === 'finnhub' ? undefined : 1)
