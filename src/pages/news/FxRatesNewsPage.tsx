...
import { fetchFx, getProvider, type NewsItem, normalizeNewsUrl } from '../../lib/news/providers'
import PreviewImage from '../../components/news/PreviewImage'

export default function FxRatesNewsPage() {
  ...
  const loadMore = React.useCallback(async () => {
    ...
    try {
      ...
      const key = provider === 'reddit' ? cursor : provider === 'finnhub' ? cursor : page
      const { items: more, cursor: nextCursor, nextPage } = await fetchFx(key)
      setItems((prev) => {
  ...
  // Hydrate from localStorage and background refresh if stale
  React.useEffect(() => {
    try {
        ...
        if (Array.isArray(c.items)) {
          ...
          if (age > CACHE_TTL_MS) {
            ;(async () => {
              ...
              try {
                const provider = getProvider()
                const key = provider === 'reddit' ? undefined : provider === 'finnhub' ? undefined : 1
                const { items: more, cursor: nextCursor, nextPage } = await fetchFx(key)
    ...
    if (items.length === 0 && !loading) void loadMore()
  }, [])
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="flex justify-end pt-2">
        <button type="button" className="h-8 px-3 text-xs rounded-full border border-input hover:bg-accent" onClick={async()=>{ setHasMore(true); setCursor(undefined); setPage(1); if(!loading){fetchingRef.current=false; await loadMore()} }}>새로고침</button>
      </div>
    </section>
  )
}
